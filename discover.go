...
func loadRegistry(globalDir string) ([]Project, error) {
	...
	// the registry path is caller-supplied by design: reading local files at arbitrary paths is this library's purpose
...
func queryProjectsCLI(ctx context.Context, binary string) ([]Project, error) {
	...
	// running the configured crush binary is the documented CLI fallback, not an injection vector
