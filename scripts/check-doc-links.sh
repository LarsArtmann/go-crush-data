...
# point at real files.
#
# Why: the 2026-08-16 docs-health audit found a used-but-undefined [0.2.0]
# link reference and stale file:line citations — drift that only a manual
# full-document read caught. This guard makes that drift class fail the
# gate instead of waiting for the next audit.
#
# Scope: root-level *.md only. docs/status, docs/planning, and docs/reviews
# are historical records whose citations intentionally describe past states.
# Fenced code blocks are excluded: their contents are examples, not links.
#
# Checks:
...
#      code spans and task-list checkboxes are excluded. (This is the exact
#      audit bug: [0.2.0] used, never defined.)
#   3. Backtick file citations (`stats.go:28`, `dir/script.sh`) resolve to
#      real files, and a given line number is within the file. Citations
#      containing a glob character (`*`) are matched with find. Crush's
#      runtime artifacts (projects.json, crush.db) are cited as storage
#      facts, not repo files, and are exempt.
#
# Usage:
...
# Runtime files Crush writes, cited in prose as storage facts — not repo
# files, never a stale-reference signal.
runtime_artifacts="projects.json crush.db"
...
for doc in ./*.md; do
	...
	done < <(grep -hoE '`[^`]+`' <<<"$prose" | tr -d '`' | grep -E '\.(go|yml|yaml|sh|md|txt|json|nix|mod|db|lock)(:[0-9]+)?$' || true)
done
